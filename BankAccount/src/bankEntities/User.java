...
public class User {
	...
	private int getAccount(int acc)
	{
		for(int i = 0; i < myAccounts.size(); i++)
			if(myAccounts.get(i).getAccNum() == acc)
				return i;
		return -1;
	}
	public boolean deposit(int accNum, int amm)
	{
		int idx = getAccount(accNum);
		if(idx == -1)
			return false;
		return myAccounts.get(idx).deposit(amm);
	}
	/******************************
